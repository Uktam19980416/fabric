...
export const WinterCollection: FC<WinterCollectionProps> = ({data}) => {
  ...
  if (title) {
    ...
    if (!item) return <div>{t("home.winterCollection.notFound")}</div>;
    ...
    <div className="max-xl:px-2 mt-10">
      <div className="container w-full max-w-[1440px] mx-auto max-sm:px-2 max-sm:w-full mt-20">
        <div className="text-center">
          <h1 className="text-5xl font-bold">{t("home.winterCollection.title")}</h1>
          <p className="text-xl font-medium text-slate-600">{t("home.winterCollection.text")}</p>
        </div>
